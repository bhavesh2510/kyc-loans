...
const Service = {
  ...
  emailOTP: (data) => {
    ...
    //   crossdomain: true,
    //   url: 'https://pay.cibopay.com/Sendotp/sendotpemail',
    //   data: data,
    // });
  },
  phoneOTP: (data) => {
  ...
  getData: (country, value) => {
    if (country === 'DK') {
      return axios.get(
        'http://cvrapi.dk/api?search=' + value + '&country=' + country
      );
    } else if (country === 'GB') {
      return axios.get(
        'https://pay.cibopay.com/getData/search/regnumber/' + value
      );
    }
  },
  getShareHolder: (compnumber) => {
    return axios.get(
      'https://pay.cibopay.com/getData/shareholderInfo/regnumber/' + compnumber
    );
  },
};
